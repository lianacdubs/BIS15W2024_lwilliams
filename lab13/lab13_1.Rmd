...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) #look in the files and iterate read_csv to each
```

We can view elements in our list with double brackets. Let's view the data for Butte county.
```{r}
spider_list[[3]] #Look at the third element in the list
```
...
names_list
#now have the names of the county associated with the [[]]
```
...
```{r}
spider_list[["Butte"]] #now instead of the number we can use the county name
```

```{r}
butte <- spider_list[["Butte"]] #store this as a temporary object so that way we can pipe. 
```

## Merging Files (for two data sets that we want to put together)
...
```{r}
table_A
#customer ID is uniquely associated with the data. 
```
...
left_exampleDF
```
#take the table and only match from the right 
...
right_exampleDF
```
#right table is now the one thats absolute and you match from the left 
...
full_exampleDF
```
#doesn't care about left or right it just cares about matches. Needs to be EXACT! 
...
```{r}
spiders_with_locs <- left_join(spiders_all, spiders_locs, by="Accession") #accession is the unique number
summary(spiders_with_locs)
#add spiders_loc to spiders_all 
#left join tag it on at the end of the data 
#order matters first one is the left the second one is the right 
```
