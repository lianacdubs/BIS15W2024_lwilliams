...
filter()    ex: filter(order=="Primates") effectively does a mutate() to generate a logical variable, and then only keeps the rows where the variable is TRUE. This means that grouped filters can be used with summary functions. 
```{r}
#Start by building a new data frame `msleep24` from the `msleep` data that: contains the `name` and `vore` variables along with a new column called `sleep_total_24` which is the amount of time a species sleeps expressed as a proportion of a 24-hour day. Restrict the `sleep_total_24` values to less than or equal to 0.3. Arrange the output in descending order.  
msleep24 <- msleep %>% 
...
#check by looking at the max and min values do summary(msleep24)
```

skim() part of skimr package that is a toolt to check what the output is. 
...
#you can add mor variables after island by adding a comma
```
For counts tabyl() also works (this is part of the janitor package and also shows counts)
```{r}
penguins %>% 
...
```
**summary of NA's in each variable**
...
**group_by also works** use it for categorical data
```{r}
penguins %>%
  group_by(sex) %>% #you can add a comma to add another variable 
  summarize(across(contains("mm"), mean, na.rm=T))
```
...
life_history <- 
  life_history %>% 
  mutate(max_life=na_if(max_life, 0)) #max life should not be 0 thats why we are replacing it
```
...
**common NA's** Finally, naniar has some built-in examples of common values or character strings used to represent NA's. The chunk below will use these built-in parameters to replace NA's across the entire data set.
```{r}
common_na_strings
```
...
Solution 1: specify a range of columns that you want to pivot. (**when you need to pivot some but not all data**)
```{r}
billboard2 <- 
  billboard %>% 
...
expenditures %>% 
  ...
  arrange(-bn_dollars) #the negative sign is the same as desc
```
...
```
The value comlumn is not represented as a single variable (cases and population)
**Examples**
...
#geom is how we want our data represented 
...
### The `mapping=` function is implied by `aes` and so is often left out. 
```{r}
...
####this is good for straight counts
...
```{r}
penguins %>% 
  filter(sex!="NA") %>% #takes out all the NAs in sex
  group_by(sex) %>% 
...
### Box Plots
For the next series of examples, we will use the `homerange` data.  The center line in each box represents the median, not the mean.
