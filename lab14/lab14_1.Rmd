...
1.  The `ui` function controls the user inputs and the way the app will be displayed; i.e. this controls how your app looks.  **like a dropdown menu** 
...
shinyApp(ui, server)
#^This is a shiny app. You can pull this up. 
```

For the rest of the lab we will work on building an example app using `palmerpenguins`.

## Shiny Inputs
**inputs is the diff types of ui parts: drop down, buttons, UI parts**
...
#select input is the dropdown menu, x, the select X variable shows up in the app, selected is where it is going to start. 
```{r, eval=F, echo=T}
ui <- fluidPage(
...
^Now the user interface is set up but, the app currently doesn't do anything! 
...
shinyApp(ui, server)
```
^make sure the input$ matches the input function! 
