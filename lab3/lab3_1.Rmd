...
```{r}
dim(hbirds)
#also in environment 3 observations and 3 variables (3by3)
```

```{r}
str(hbirds)
#this gives the structure where each variable(column) is NOW a row 
#an observation is what goes in a cell. 
```
...
```
^
####the first in the = is the new and the second is the old
####the hbirds will overwrite the data we had
...
```{r}
hbirds$weight_g
#this gives the weight column 
#you can also save as a variable like w <-
```
...
```{r}
#you can change the order for the data frame like 
...
hsprings
#takes the vectors and stacks them on top of eachother.
```
...
hsprings
#rebuild the dataframe. rewrite the hspring data. new name = old name. 
```
