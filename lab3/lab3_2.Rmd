...
```{r}
names(fish) #gives all the names of the fish
```
...
```{r}
summary(fish) #gives basic stats but, cannot do that for a number that is not numeric
```
...
```{r}
table(fish$lakeid) #does not always work so, you have to specify what to look at. (shows states and numbers per state)
```

We can also click on the `fish` data frame in the Environment tab or type View(fish).
```{r}
#View(fish) #same as the spreadsheet in the top right of environment panel
```
...
little_fish
#needed for hw look in fish dataframe and pull out small fish
```
