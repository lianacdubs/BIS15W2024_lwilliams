...
Note: In ggplot we are NOT piping anything we are adding layers. 
...
In general you can keep the data as a simple chart! 
...
```{r}
glimpse(iris) #note: one variable is a factor
```
...
```{r}
ggplot(data = iris, 
       mapping = aes(x=Sepal.Width, y=Sepal.Length)) + #plots are built in layers
  geom_point() #use data visualization cheat sheet in lab10
```
...
###geom_smooth(method = lm, se=T, na.rm = T) ####FIX THIS CODE 
```
