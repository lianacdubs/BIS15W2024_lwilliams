...
```{r}
penguins %>% 
  count(island) #island is x and the n is y 
```
...
```{r}
penguins %>% 
  count(species) #for a straight count use geombar 
```
...
Let's look at the variable `log10.mass` grouped by class.
```{r}
homerange %>% 
  group_by(class) %>% #take all the class and put them together 
...
```{r}
homerange %>% 
  group_by(trophic.guild) %>% #take all the trophic and put them together 
  summarize(mean_log10.mass=mean(log10.mass)) #summarize to get x and y 
```
...
homerange %>% 
  ...
  ggplot(aes(x=trophic.guild, y=mean_log10.mass))+
  geom_col()
...
homerange %>% 
  ggplot(aes(x=trophic.guild, y=log10.mass))+ #continuous vs categorical variable
  geom_boxplot()
```
