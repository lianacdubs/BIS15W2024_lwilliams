...
homerange %>% 
  ggplot(data=homerange, 
         mapping=aes(x = class, y=log10.mass, group=taxon))+ #we don't know what the boxes represent
  geom_boxplot()
#group (different categorical variable than class) but, no color added 
```
...
homerange %>% 
  ggplot(data=homerange, 
         mapping=aes(x = class, y=log10.mass, fill=taxon))+ #fill by taxon is a different categorical variable
  geom_boxplot()
```
...
deserts %>% 
  ...
  pivot_wider(names_from = species_id, values_from = n) #makes data nicer since species id has either DM or DS
...
#histogram with density plot overlayed!
```
...
homerange %>% 
  ...
  labs(title = "Distribution of Homerange")
```
Note: Use the same color and the same fill consistently throughout! 
