...
Next, the y-axis is not on a log scale. We can fix this by adding `scale_y_log10()`. (some have really large masses and some are small)
```{r}
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass = mean(mass, na.rm = T)) %>% 
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height", #this adds title 
       x = "Age (years)",
       y = "Height (cm)") 
```
