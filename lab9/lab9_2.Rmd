...
```
#the years are column names. we need an X or we need a y. 
...
#consequence is the data is longer but, now we can make a plot. 
...
Problems with ^ data 
-Column names are the problem 
-sex and pateint name in the same column
...
heartrate3 <- heartrate2 %>% 
  separate(patient, into=c("patient", "sex"), sep="_") %>%  #separate 
  pivot_longer(-c(patient, sex),
               names_to = "drug", #pull drugs apart 
               values_to = "heartrate")
heartrate3
```
...
```{r}
tb_data %>% 
  filter(key=="population") #now we have all the population only and then you could do something with this analysis
#but, you can't do anything with this data out of the gate. 
```
...
tb_data
```
#key is the issue and the values are not the same. The value comlumn is not represented as a single variable.
...
beachbugs_wide
```
#buglevels is the concentration of bacterial contamination(result of fecal contamination) of these beaches. 
