...
```{r}
penguins %>% 
  filter(species=="Adelie") %>% #references the column name Adelie 
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct)) #concatenate the variables of interest, get counts
```

This is very helpful for continuous variables.
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #summarize across all variables that have mm in the name.
```
...
```{r}
penguins %>%
  summarize(across(!c(species, island, sex, year), #summarize across all the variables that are not these
                   mean, na.rm=T))
```
...
penguins %>% 
  group_by(species) %>% 
  summarize(across(c(contains("mm"), body_mass_g), mean, na.rm=T), #notice that i can include variable other than the ones that have mm
            n_samples=n())

#To do sd just switch from mean to sd 
```
