...
levels(fish$lakeid)# gives all the possible values within the variable 
```
#A factor: example; looked at tropic_guid either herbivor or carnivore. (two values for the variable)
# Factors are data type to use when you know a certain variable will have repeated values. so when you take the lvels of a factor, you are shown all the different levels (values). 
#Factors show how many categories there are in a variable.
Now we can pull out any state of interest.  
```{r}
filter(fish, lakeid=="LJ") #pull all observations for lakeid LJ 
```
